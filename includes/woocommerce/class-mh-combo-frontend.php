...
class MH_Combo_Frontend {
    ...
    public static function add_to_cart_template() {
        ...
        if ( ! is_array( $combo_product_ids ) || empty( $combo_product_ids ) ) {
            echo '<p class="mh-combo-empty">' . esc_html__( 'No items are included in this combo yet.', 'mh-plug' ) . '</p>';
            return;
        }
        ...
        echo '<h4 style="margin-top: 0; margin-bottom: 15px; font-size: 16px;">' . esc_html__( 'This Combo Includes:', 'mh-plug' ) . '</h4>';
        ...
        echo '<button type="submit" name="add-to-cart" value="' . esc_attr( $product->get_id() ) . '" class="single_add_to_cart_button button alt mh-combo-btn">' . esc_html__( 'Add Combo to Cart', 'mh-plug' ) . '</button>';
    ...
    public static function render_custom_variations_dropdowns() {
        ...
        foreach ( $all_attributes as $attribute ) {
            ...
            echo '<option value="">' . esc_html__( 'Choose an option', 'mh-plug' ) . '</option>';
