...
(function ($) {
    ...
    $(document).on('click keydown', '.mh-wishlist-btn', function (e) {
        ...
        // Skip if inside product grid — handled by mh-widgets.js
        if ($btn.closest('.mh-product-grid').length) return;
        ...
        $.ajax({
            ...
            method: 'POST',
            data: {
                action:     'mh_wishlist_toggle',   // unified toggle action
                security:   nonce,
                product_id: productId
            },
            success: function (res) {
                if (!res.success) {
                    if (window.mhShowToast) window.mhShowToast(res.data && res.data.message ? res.data.message : 'Failed to update wishlist', 'error');
                ...
                if (window.mhShowToast) {
                    if (wasAdded) {
                        window.mhShowToast('Product added to wishlist', 'success');
                    } else {
                        window.mhShowToast('Product removed from wishlist', 'info');
                    }
                }
            },
            error: function () {
                if (window.mhShowToast) window.mhShowToast('Server error. Please try again later.', 'error');
    ...
    $(document).on('click', '.mh-wl-remove-btn', function (e) {
        ...
        $.ajax({
            ...
            success: function (res) {
                if (!res.success) {
                    if (window.mhShowToast) window.mhShowToast(res.data && res.data.message ? res.data.message : 'Failed to remove from wishlist', 'error');
                    return;
                }
                ...
                if (window.mhShowToast) window.mhShowToast('Product removed from wishlist', 'info');
            },
            complete: function () {
