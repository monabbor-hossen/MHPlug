...
class MH_Synced_Slider_Widget extends Widget_Base {
    ...
    protected function register_controls() {
        ...
        $this->add_control('meta_color', ['label' => __('Color', 'mhds-plug'), 'type' => Controls_Manager::COLOR, 'selectors' => ['{{WRAPPER}} .mh-text-content .mh-meta' => 'color: {{VALUE}};']]);
        ...
        $this->add_control('heading_color', ['label' => __('Color', 'mhds-plug'), 'type' => Controls_Manager::COLOR, 'selectors' => ['{{WRAPPER}} .mh-text-content h2' => 'color: {{VALUE}};']]);
        ...
        $this->add_control('price_color', ['label' => __('Color', 'mhds-plug'), 'type' => Controls_Manager::COLOR, 'selectors' => ['{{WRAPPER}} .mh-text-content .mh-price' => 'color: {{VALUE}};']]);
    ...
    protected function render() {
        ...
        // Safely extract JS settings with solid fallbacks
        $t_speed = isset($settings['transition_speed']) && is_numeric($settings['transition_speed']) ? absint($settings['transition_speed']) : 500;
        $a_speed = isset($settings['autoplay_speed']) && is_numeric($settings['autoplay_speed']) ? absint($settings['autoplay_speed']) : 3000;
        $auto    = (isset($settings['autoplay']) && $settings['autoplay'] === 'yes') ? 'true' : 'false';
        ?>

        <style>
            /* 🚀 THE BASE FOUNDATION: Exactly as it was before, ensuring it NEVER breaks.
               Elementor's settings will safely layer on top of this. */
            .mh-synced-container {
            ...
            .mh-image-slider .slick-arrow {
                position: absolute; bottom: -60px; top: auto; transform: none; width: 50px; height: 50px;
                border: 2px solid rgba(255,255,255,0.2); border-radius: 50%; transition: all 0.2s; z-index: 100; cursor: pointer; background: transparent;
            }
            .mh-image-slider .slick-arrow:hover { background: rgba(255,255,255,0.1); border-color: white; }
            .mh-image-slider .slick-arrow:before { color: white; font-size: 24px; opacity: 1; }
