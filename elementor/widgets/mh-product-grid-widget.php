 ...
 * Beautiful Sbmart-style product cards with advanced query filters.
 */
...
class MH_Product_Grid_Widget extends Widget_Base {
    ...
    public function get_keywords() { return [ 'woocommerce', 'product', 'grid', 'loop', 'best selling', 'top rated' ]; }
    ...
    protected function register_controls() {
        ...
        $this->add_responsive_control( 'grid_gap', [
            'label'     => __( 'Grid Gap (Spacing)', 'mh-plug' ),
        ...
        $this->add_control( 'card_bg', [
            'label'     => __( 'Card Background', 'mh-plug' ),
        ...
        $this->add_control( 'card_radius', [
            ...
            'type'      => Controls_Manager::SLIDER,
            'default'   => [ 'size' => 10 ],
            'selectors' => [ '{{WRAPPER}} .mh-product-card' => 'border-radius: {{SIZE}}px;' ],
        ] );
    ...
    protected function render() {
        if ( ! class_exists( 'WooCommerce' ) ) {
            echo '<div class="mh-error">Please activate WooCommerce.</div>';
            return;
        }
        ...
        if ( ! $loop->have_posts() ) {
            echo '<p>' . esc_html__( 'No products found.', 'mh-plug' ) . '</p>';
            return;
        }
        ...
        <style>
            ...
            .mh-product-card { background: #fff; overflow: hidden; position: relative; transition: all 0.3s ease; display: flex; flex-direction: column; }
            .mh-product-card:hover { box-shadow: 0 10px 25px rgba(0,0,0,0.08); transform: translateY(-3px); }
            ...
            /* Hover Action Buttons (Sbmart Style) */
            .mh-product-actions { 
                position: absolute; bottom: -50px; left: 0; width: 100%; display: flex; justify-content: center; gap: 8px; 
                opacity: 0; transition: all 0.4s cubic-bezier(0.25, 0.8, 0.25, 1); z-index: 3; padding-bottom: 20px;
            }
            ...
            .mh-action-btn { 
                ...
                color: #333; font-size: 15px; box-shadow: 0 5px 15px rgba(0,0,0,0.1); transition: 0.3s; text-decoration: none; border: none; cursor: pointer;
            }
            .mh-action-btn:hover { background: #d63638; color: #fff; }
            .mh-action-btn.added { background: #d63638; color: #fff; }
        ...
        <div class="mh-product-grid">
            ...
            while ( $loop->have_posts() ) : $loop->the_post();
                ...
                // Integrate with our custom Wishlist system!
                ...
                <div class="mh-product-card">
                    <div class="mh-product-image-wrap">
                        ...
                        <?php if ( $settings['show_badge'] === 'yes' && $product->is_on_sale() ) : ?>
                            <div class="mh-product-badges">
                                <span class="mh-badge"><?php esc_html_e('Sale', 'mh-plug'); ?></span>
                            </div>
                        <?php endif; ?>
                    ...
                    <div class="mh-product-info">
                        <?php if ( $settings['show_category'] === 'yes' ) : ?>
                            <div class="mh-product-cat"><?php echo wc_get_product_category_list( $product_id, ', ' ); ?></div>
                        <?php endif; ?>
        ...
        <script>
            jQuery(document).ready(function($){
                ...
                $('.mh-product-grid .mh-advanced-wishlist-btn').off('click').on('click', function(e){
                    ...
                    $.post(mhAjaxUrl, { action: 'mh_wishlist_toggle', product_id: pid, security: mhNonce }, function(response) {
