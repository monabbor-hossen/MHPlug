...
class MH_Plug_Taxonomy_Slider_Widget extends \Elementor\Widget_Base {
    ...
    protected function render() {
        ...
        $show_desktop = absint( $settings['slides_to_show'] ?? 3 ) ?: 3;
        $show_tablet  = absint( $settings['slides_to_show_tablet'] ?? 2 ) ?: 2;
        $show_mobile  = absint( $settings['slides_to_show_mobile'] ?? 1 ) ?: 1;
        
        $scroll_desktop = absint( $settings['slides_to_scroll'] ?? 1 ) ?: 1;
        $scroll_tablet  = absint( $settings['slides_to_scroll_tablet'] ?? 1 ) ?: 1;
        $scroll_mobile  = absint( $settings['slides_to_scroll_mobile'] ?? 1 ) ?: 1;
        ...
        foreach ( $breakpoints as $bp ) {
            if ( $bp['media'] ) $css .= $bp['media'] . " {\n";
            $p = $bp['prefix'];
            $css .= "{$p}-left .mh-tax-card-content { align-items: flex-start !important; text-align: left !important; }\n";
            $css .= "{$p}-left .mh-tax-card-content > * { text-align: left !important; }\n"; 
            $css .= "{$p}-center .mh-tax-card-content { align-items: center !important; text-align: center !important; }\n";
            $css .= "{$p}-center .mh-tax-card-content > * { text-align: center !important; }\n";
            $css .= "{$p}-right .mh-tax-card-content { align-items: flex-end !important; text-align: right !important; }\n";
